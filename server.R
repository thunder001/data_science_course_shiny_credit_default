...
shinyServer(
  function(input, output) {
    output$income <- renderPrint({if (input$goButton!=0) input$id1})
    output$balance <- renderPrint({if (input$goButton!=0) input$id2})
    output$stu_status <- renderPrint({if (input$goButton!=0) input$id3})
    output$prediction <- renderPrint({
      if (input$goButton==0) {
        "You haven't enter your information"
      }
      else myfunction(input$id1,input$id2,input$id3)})
    }
)